...
/// (`border_colors`), instead of the default `min(5, colors-1)`. This lets a caller
/// keep Eternity II's interior-dominant colour ratio on small boards, where the
/// default cap of 5 border colours would otherwise starve the interior (e.g. at
/// 8×8, total 6 colours would give 5 border + 1 interior — a near-uniform interior
/// sea, the opposite of E2's 5:17 split). `border_colors` is clamped to
/// `[1, colors-1]`. Passing `frame_color_count(colors)` reproduces
/// `generate_solved_framed` exactly.
