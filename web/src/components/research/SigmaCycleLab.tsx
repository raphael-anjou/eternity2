...
const T = {
  en: {
    ...
    note: "Two solutions, the permutation between them, its cycles, and every score here are computed live by the WebAssembly engine on a freshly generated puzzle — nothing precomputed. On the full 16×16 the analogous cycle spans up to 154 cells and the same all-or-nothing property holds across the three record pairs tested, though that is not proven to hold in general; here it is small enough to watch.",
  ...
  fr: {
    ...
    note: "Les deux solutions, la permutation entre elles, ses cycles et chaque score ici sont calculés en direct par le moteur WebAssembly sur un puzzle fraîchement généré — rien de précalculé. Sur le 16×16 complet, le cycle analogue couvre jusqu'à 154 cellules et la même propriété tout-ou-rien tient sur les trois paires de records testées, sans qu'il soit prouvé qu'elle vaille en général ; ici il est assez petit pour être observé.",
  ...
  es: {
    ...
    note: "Las dos soluciones, la permutación entre ellas, sus ciclos y cada puntuación aquí se calculan en vivo mediante el motor WebAssembly sobre un puzzle recién generado — nada precalculado. En el 16×16 completo el ciclo análogo abarca hasta 154 celdas y se cumple la misma propiedad de todo o nada en los tres pares de récords probados, aunque no está demostrado que valga en general; aquí es lo bastante pequeño para poder observarlo.",
