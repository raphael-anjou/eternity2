...
description: >-
  ...
  with small worked sketches. A measurement on planted boards puts a number on
  the choice of formulation: a solvability cliff one search paradigm hits and
  another crosses, and that moves with the colour count.
metaDescription: "Edge matching is NP-complete as a family, but one fixed 16x16 board is a constant, not a problem. Alongside SAT, exact-cover DLX, and ILP encodings, a planted-board measurement shows a solvability cliff that DFS hits and CP-SAT crosses, and that moves with the colour count for both paradigms."
kind: concept
...
repro:
  ...
  produces: search
  scope: "Reruns both arms of the planted-board cliff measurement (the restarting DFS sweep and the CP-SAT decision solves with independent board verification); per-seed scores and solve counts reproduce, wall-clock times vary with hardware."
---
...
## How much does the formulation matter? A measured cliff
