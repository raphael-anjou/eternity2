...
repro:
  ...
  scope: >-
    El comando reproduce exactamente, en el repositorio: la cota horizontal
    lambdaH y las densidades de entropía h(1), h(2) para los anchos 1 y 2, a
    partir de los valores propios de la matriz de transferencia; los recuentos
    de bloques interiores A(n) hasta n=4 y los recuentos con piezas distintas
    B(n) hasta n=3 (B(2)=4059952 coincide con la tabla subgrid-placement-counts);
    y el exponente de ley de área alpha cercano a 0,044 ajustado sobre ese rango.
    Los puntos de mayor ancho h(3), h(4) y el límite h_infinity en torno a 0,67
    siguen provenientes de un barrido fuera de línea, y la curva rho(n) y el
    umbral de las ~155 celdas más allá de n=3 son extrapolaciones del ajuste del
    repositorio, no recuentos exactos.
sources:
...
<Figure caption="Fracción de bloques compatibles en colores realizables con piezas distintas. El exponente es el ajuste por mínimos cuadrados del repositorio sobre los bloques contados con exactitud (n hasta 3); la curva más allá es una extrapolación.">
  <RhoChart />
</Figure>
