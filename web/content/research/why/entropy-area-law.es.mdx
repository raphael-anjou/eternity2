...
repro:
  ...
  scope: >-
    El comando reproduce exactamente la mitad de la entropía de la gramática:
    la cota horizontal lambdaH y las densidades de entropía h(1), h(2) para los
    anchos 1 y 2, a partir de los valores propios de la matriz de
    transferencia. Los puntos de mayor ancho h(3), h(4), el límite h_infinity
    en torno a 0,67, el exponente de ley de área alpha en torno a 0,085, el
    umbral de las ~80 celdas y el recuento de bloques n=3 del laboratorio
    interactivo provienen de un estudio fuera de línea y no los regenera este
    comando.
sources:
...
El exponente $\alpha \approx 0.085$ y el umbral de las ochenta celdas de más
abajo proceden de un ajuste sobre una sola instancia obtenido de un estudio
fuera de línea; el comando de reproducción de esta página no los recalcula
(cubre la mitad exacta, la de los valores propios).
...
<Figure caption="Fracción de bloques compatibles en colores realizables con piezas distintas. El alpha y el umbral de las ochenta celdas se miden fuera del sitio, no los recalcula aquí el comando de reproducción.">
  <RhoChart />
</Figure>
...
0,67, y el exponente de ley de área $\alpha \approx 0.085$, son ajustes sobre
una sola instancia propios de este proyecto, extrapolados más allá de los
anchos contados exactamente; el teorema no fija esos valores.
