...
metaDescription: "Why a faster computer doesn't help on Eternity II: shrinking the search space beats searching faster by an exponential margin, while a speedup is just a constant divisor. E2 is tuned so almost nothing local can be pruned, which is why even legal pruning rarely pays and no engine has reached 480."
kind: finding
...
- **[The area law](/research/why/entropy-area-law)**: the count of
  genuinely-distinct partial boards collapses as the filled area grows, but no local
