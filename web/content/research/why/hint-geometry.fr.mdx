...
description: >-
  ...
  dispersés sur le plateau le résolvent en quelques minutes, là où le même
  casse-tête réclame quatre-vingts indices ou plus, entassés dans des rangées
  contiguës, pour devenir aussi facile. C'est la position, non le nombre, qui
  fait levier, et elle pointe droit vers la fin de partie.
metaDescription: "Sur un casse-tête 16×16 façon Eternity II, la position des indices prime sur le nombre : 18 indices dispersés le résolvent en minutes, quand 80+ entassés en rangées peinent à égaler, car 99 % de la recherche a lieu après la profondeur 132 sur 256. Seuls comptent ceux atteignant la fin de partie."
kind: finding
...
indices posés aux bons endroits le résolvent en quelques minutes ; le même
casse-tête, lui, en réclame quatre-vingts ou plus, si on les entasse dans des
rangées contiguës, pour être aussi facile. Un facteur quatre sur le nombre,
tranché entièrement par la géométrie.
...
41 160 067 167 placements. Descendre à quinze indices marchait encore ; la
recherche gonflait simplement à plusieurs heures. Entassez au contraire les
...
à partir d'une solution connue, et il lui en fallait bien davantage avant que le
casse-tête ne cède :
...
<div className="grid grid-cols-2 gap-3 text-center sm:grid-cols-4">
  ...
  <div className="rounded-lg border p-4">
    ...
    <div className="mt-1 text-xs text-muted-foreground">indices en rangées contiguës pour une facilité comparable</div>
  </div>
  <div className="rounded-lg border p-4">
...
<Callout kind="note">
  ...
  avec attribution plutôt que redérivés.
</Callout>
