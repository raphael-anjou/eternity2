...
board locally, and (on the three record pairs measured here) you can't hop to a
neighbour either, because the nearest better board is one indivisible move of
80 to 154 cells away. These cycles are measured to McGavin's 469; reading the
same wall up to the standing 470 extrapolates one edge further. This is a
...
specific record-board cycle counts on this page are not yet shipped as a
committed, re-runnable artifact here; the interactive Lab above computes the
same mechanism live on freshly generated small puzzles instead. The animation
above is a schematic of the mechanism, not the 154-cell cycle itself.*
