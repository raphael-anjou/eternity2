...
metaDescription: "Brendan Owen's complex theory estimates how wide the Eternity II search tree is at every depth, and how many solutions exist at all. It reveals the funnel: a vast plateau where backtrackers spend 99% of their time, and shows why scan order, not raw speed, decides a solve."
kind: finding
...
sources:
  ...
  - label: 'Brendan Owen''s heuristic-vs-node-count study, "Heuristics: 20×2 rectangle" (groups.io Files, Brendan folder)'
    url: https://groups.io/g/eternity2/files/Brendan/heuristics.pdf
  - label: "Joe's backtracker depth-time measurement, 1B-iteration sample (groups.io msg 11725, 2026)"
...
counter: it climbs astronomically, then barely moves for a hundred cells
...
- **Plateau (depth 50–200).** The tree is at its widest, about $10^{45}$
  ...
  backtrackers spend the bulk of their time: one empirical sample
  ([groups.io msg 11725](https://groups.io/g/eternity2/message/11725), a
  1B-iteration run) found 99% of backtracker time at depth $>132$ and 70% at
  depth $>150$, i.e. deep in the tree rather than early.
...
distinctness collapses as the filled area grows, a second-order effect the
