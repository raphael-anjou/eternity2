...
description: >-
  ...
  that count of around 17 lands near where this kind of puzzle is hardest to
  solve (the transition is a band, not a single integer).
metaDescription: "Eternity II's 22 colors split 17 interior to 5 frame-only, and that count of around 17 sits near the phase transition where edge-matching puzzles are hardest (a band, not a single integer). Brendan Owen derived the 17+5 design to be maximally hard; later SAT/CSP research places the peak nearby."
kind: finding
...
repro:
  ...
  scope: "Reproduces the 17/5 interior/frame color split. The hardness-peak location is separately measured in-repo by the hardness-peak experiment (an 8x8 solver-effort sweep plus Owen's criterion recomputed to 17.14 at size 16); the 'one expected solution' claim for the full 16x16 board still rests on the cited external sources (Owen msg 1947, Ansótegui/Mateu)."
---
...
We measured this here. Owen's criterion was only ever stated for the full board,
so we ran it through a second size to see whether it holds. The same formula
predicts $(36! \cdot 4^{36})^{1/72} \approx 7.56$ interior colours for an 8x8, and
an 8x8 is small enough that a plain backtracker actually solves random instances,
so its difficulty curve can be swept directly. Sweeping solver effort against
interior-colour count on generated 8x8 boards (the site's own generator and DFS)
puts the hard band, where the search stops finding a solution within budget, right
where the size-8 criterion predicts. The full 16x16 sweep is the other half of the
picture: a plain backtracker never completes it at any colour count, and the depth
it reaches falls steadily as colours rise, so the real board sits past the point
where the curve has already gone vertical. The
[hardness-peak experiment](https://github.com/raphael-anjou/eternity2/tree/main/research/topics/hardness-peak)
holds the code, both sweeps, and the criterion recomputed in-repo (it reproduces
Owen's 17.14 at size 16).
