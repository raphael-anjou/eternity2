...
records, methods, structural findings, honest dead ends — gets written down,
sourced, and kept findable. Raphaël's experiments live here under "Raphaël's
explorations" because that is one researcher's corner, not the whole house.
Yours can sit right beside it.
...
page, run it past you, and publish it with full credit — exactly how the
Blackwood page came to exist from Jef's notes. You never have to touch the
repository.
...
topics: [backtracking]
sources:
  - label: Where the claim comes from
...
- **Every number is reproducible or honestly labelled.** Deterministic results
  come with a command that regenerates them exactly. Seeded, stochastic, or
  heavy computations say so plainly — "won't reproduce exactly", "~40 h on
  8 cores" — and ship the board they found so it can still be verified.
...
can cite — years from now, not just this week.
