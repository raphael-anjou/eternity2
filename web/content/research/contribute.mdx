...
metaDescription: "Publish your Eternity II research on this community wiki: three routes, from a mailing-list post to a pull request, so a maintainer or you can give it a permanent, citable home. Plus the house rules — every claim sourced, every experiment listing its hardware — that keep pages honest."
kind: tool
order: 70
updated: 2026-07-21
topics: []
...
<Callout kind="note" title="Nothing to publish yet?">
  Start by building. The [builder's toolkit](/research/build/toolkit) gets you
  to a running solver, and for a first target with a name to make, Brendan
  Owen's
  [10×10 set_2](/research/build/benchmarks#brendans-1010s-one-fell-one-is-still-standing)
  is the nearest unsolved board below the full puzzle. A result there is worth
  bringing back here.
</Callout>
