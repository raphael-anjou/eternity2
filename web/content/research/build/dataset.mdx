...
description: >-
  ...
  instances to solve, and a corpus of 1,078 distinct strong boards to learn
...
metaDescription: "A public CC0 Eternity II dataset: 14 benchmark instances to solve plus 1,078 distinct verified strong boards to learn from, with a diversity analysis."
kind: reference
order: 55
...
**1,078 distinct boards** scoring 400 to 469, the raw material for methods that
...
(`e2-strong-boards.zip`, about 620 KB) that unpacks to the same rows as CSV and
as JSON-lines:
...
which basin it sits in; there are 28 distinct families. The score distribution is
a clean bell curve peaking at 452 to 456, with 21 boards at 460 or above and the
best at 469.
...
A pile of a thousand boards is only worth publishing if the boards are actually
different. Before releasing this one we checked that they are not just
perturbations of a single board, and they are not:

- Only 11 of the 1,089 source boards were exact duplicates; the rest are
  distinct, leaving 1,078.
- The *median* board differs from its closest sibling by 60 of its 256 piece
  placements. Only 3% of boards sit within 10 placements of another, and only 7%
  within 25.
...
matters for anything that mines it: a learned prior built from a thousand copies
of one board would encode that board, not the structure of good boards in
general.
