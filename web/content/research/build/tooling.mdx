...
sources:
  ...
  - label: e2.bucas.name (Jef Bucas) — the community viewer, today's shared board format
...
now — with the [history pages](/research/build/history) telling the story
these tools appear in, and the [solver pages](/research/build/solvers)
covering the algorithms. Dead links are stated as dead; nothing here is a
...
([msg 7578](https://groups.io/g/eternity2/message/7578)) — and the
moderators removed piece files from uploaded tool archives when authors
slipped ([msg 7609](https://groups.io/g/eternity2/message/7609)). That is
...
launch — the seed of the entire benchmark culture
([msg 41](https://groups.io/g/eternity2/message/41)) — and Sergio "Zerjillo"
...
any 16×16 board or 256-piece set from the official edge numbering — shipping
the *renderer* rather than the piece data, precisely to stay on the right
...
([msg 3541](https://groups.io/g/eternity2/message/3541)). And in February
...
itself has [its own solver page](/research/build/solvers/verhaard-eii).
...
- **Eternity II Editor** grew from editor to solver: 1.3 in March 2009,
  ...
  Bruno Gauthier's 460 — still the best board honouring all five clues —
...
- **e2walker** — Ole Knudsen's fill-path explorer, in the group Files
  section since August 2007 and, when the list took inventory in 2009,
  still "the only program of its kind": a solver that lets you specify an
  arbitrary 256-step fill path
...
transcription of Owen's model, went into the group Files in 2013
...
([msg 10095](https://groups.io/g/eternity2/message/10095)). The web got its
...
messages, files and photos across with days to spare
...
[Blackwood solver page](/research/build/solvers/blackwood) covers the
algorithm itself.

The rest of the modern shelf: mtmidgee's **E2Play**, a VB.NET manual-play
board open-sourced from Melbourne lockdown — "not a solver in any way shape
or form" ([msg 10208](https://groups.io/g/eternity2/message/10208)); David
...
Dobrogost's **eii-pgen** (github.com/michal-dobrogost/eii-pgen), a
...
cleanup produced the reference piece list in the de-facto standard
numbering that newcomers are pointed to today
([msg 10970](https://groups.io/g/eternity2/message/10970)). The numbering
...
URL: width, height, piece list and rotations in the fragment, rendered
...
[the notable boards](/research/lab/boards).
...
| e2walker (fill-path explorer) | Ole Knudsen | 2007-08 | group Files (members) | [7056](https://groups.io/g/eternity2/message/7056) |
...
| eii-pgen (generator + C library) | Michal Dobrogost | 2026-01 | GitHub (michal-dobrogost/eii-pgen) | [11752](https://groups.io/g/eternity2/message/11752) |
