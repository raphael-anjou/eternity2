...
solves fast and reports a nearly perfect board — made of fractions of
...
| 2007 | dmitri_ulitski | ILP (glpsol) on rotation sets | Solved — ~5 s per set; the subproblem is easy | [3320](https://groups.io/g/eternity2/message/3320) |
...
fix only each piece's orientation so the directional edge counts balance —
fell to `glpsol` in five seconds in 2007 and to CPLEX in milliseconds in
...
- **The LP: polynomial, honestly fast.** Interior-point methods solve
...
The honest conclusion is relocation, not dismissal — the same one the
...
- **Easy integer subproblems, dispatched instantly.** Rotation sets by MILP
  ...
  is the fastest honest way to settle it — including settling that the
  answer doesn't help.
