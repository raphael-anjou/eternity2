...
much further — domains are tight, single deletions strand candidates two and
three cells away. That is exactly the behaviour on the real puzzle: arc
...
2. **A piece lands on B2.** Its domain collapses to the single placed
   candidate. Every arc *aimed at* B2 — one per neighbour, here (B1→B2),
   (C2→B2), (B3→B2), (A2→B2) — enters the queue. Nothing else does: no other
...
4. **Deletions re-arm arcs.** B1's domain shrank, so candidates elsewhere
   ...
   — except the one from B2 that just fired — re-enters the queue. This is
   the ripple.
...
6. **Fixed point.** The queue is empty: every candidate everywhere has a
   ...
   final state — the fixed point is unique; the queue discipline only changes
