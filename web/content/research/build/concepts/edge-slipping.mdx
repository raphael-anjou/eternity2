...
description: >-
  ...
  astronomically — the reason his 467 was found more than fifty times, and
...
In the summer of 2008, Verhaard was stuck the honest way. Without any
...
The allowance is depth-gated and cumulative — what his documentation calls
the *slip array*: for every depth, the number of slipped edges the search is
permitted to have accumulated so far. Below the first gate the solver is an
ordinary exact backtracker. Past it, each placement may either fit perfectly
or, if the budget at that depth allows, half-fit — and the budget unlocks
one slip at a time as the depth increases. Two facts make late gates the
whole game:
...
2. **Slide to N = 2.** Already ≈ 1.8 × 10⁵ times more targets, at score 478.
   Each extra allowed slip multiplies the class again — the bars climb by
...
- **The cost of one more matched edge.** Consecutive ratios telescope:
  ...
  factor of about sixty — matching the observed behaviour of Verhaard's
  solver, Max's own measurement of a ~30×-plus slowdown per edge, and
  Owen's older ~100× rule of thumb
  ([msg 6390](https://groups.io/g/eternity2/message/6390)).
...
arrived in 2020 — an outsider, his 468 relayed to the list from Reddit — his
record solver carried a hard-coded list:
...
edges, the winning move — twice, thirteen years apart — was not a better
