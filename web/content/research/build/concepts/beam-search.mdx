...
$O(Kb \log Kb)$ sort per level). Both are linear in $K$ — that is the whole
...
[dead ends page](/research/build/dead-ends). The honest reading of
$O(K \cdot b \cdot d)$ is therefore not "cheap" but "only as good as its
scoring function": the beam evaluates a vanishing fraction of the tree, and
