...
topics: [structure]
sources:
  - label: "Brendan Owen's launch-day estimate — 5,930 with the hint, ~4.6M without, reconciling Monckton's 'about 5 million' (msg 987, 2007-07)"
...
nevertheless agrees — to within a claimed factor of two — on how many exist:
...
reverse-engineered the designer's knob within days of the 2007 announcement,
...
one piece mistyped. The 4×4 corners followed in 2009, cross-verified by
...
benchmark puzzles were exhausted — about $1.9\times10^{14}$ and
$1.45\times10^{14}$ nodes, three weeks each on a dual-core Opteron —
yielding **2** and **3** solutions where
[complex theory](/research/why/complex-theory) had predicted 3.2
([msg 8793](https://groups.io/g/eternity2/message/8793)). Even the counting
...
mandatory hint and ≈ **4.65 million** without it — and used the split to
audit the marketing: Christopher Monckton had told him the designers
expected "about 5 million", so Owen concluded their mathematicians had
simply forgotten the hint constraint
([msg 987](https://groups.io/g/eternity2/message/987)). David Eddy added
the same day that parity corrections — every colour's edge count must be
even — contribute a factor of roughly $2^{16}$ that brings the estimate
...
2007 paper argued for ~**15 million** solutions with the starter — and got
genuine peer review on the list, with mjqxxxx catching Monte-Carlo flaws
that produced a corrected second version
([msg 3497](https://groups.io/g/eternity2/message/3497),
[3583](https://groups.io/g/eternity2/message/3583)). A "20,000 solutions"
...
a solution*, and generating-then-keeping oversamples "friendly" piece sets
with many solutions — so the true count, conditioned on the generation
process, was argued to sit anywhere from ~1,000 to ~20 million
...
depth — and each of the four independently estimated **≈ 4.05×10³⁷** border
...
"exactly one" line. Scale the same product up to the real board — $4!$
...
And here honesty is due: **0.02 is not 14,702**. The naive product is
