...
description: >-
  ...
  by orders of magnitude. Twenty years of community science — the
  fixed-vs-dynamic wars, the strategy races, the magic 10×16 square,
  Verhaard's comb search — all answer the same question: which path through
  the board is cheapest?
kind: concept
order: 110
updated: 2026-07-02
...
sources:
  ...
  - label: The fixed-vs-dynamic debate — fixed paths win empirically (msg 2392; Owen's verdict msg 2425)
    url: https://groups.io/g/eternity2/message/2392
  - label: istarinz's one-word answer to why fixed paths win — speed (September 2008)
  ...
  - label: Owen's design experiment — scanline wins only because E2's 5/17 split is balanced (msg 5263; tables msg 5243)
  ...
  - label: Max's near-identical high-score order — twelve rows, then columns (October 2008)
...
built at depth $D$ — so you can score any candidate order, shape by shape,
without running it. His conclusion from doing this exhaustively: the shapes
...
Txibilis struck back within two days with 85,729, leaving doc_s_smith
musing about why humans are so strong in this discipline
([msg 2928](https://groups.io/g/eternity2/message/2928)). The lesson that
...
attackable: middle-first beats scanline by ~100× on the 2/19 design,
...
the orientation that connects the mandatory starter piece earliest, and the
one Verhaard already used — came out around $2.3\times10^{40}$
...
then column scan — and reported scores about one edge below "what Louis
solver accomplishes"
...
5. **Lengthen the teeth.** More of the board moves into vertical mode and
   the weak placements multiply — Verhaard's trade stated visually: the
...
- **Between order families, orders of magnitude.** Middle-first on E2 is
  ...
  order costs ~70× over 1×1 scanline by the same estimates that the magic
  10×16 rule summarizes ([msg 5868](https://groups.io/g/eternity2/message/5868)).
