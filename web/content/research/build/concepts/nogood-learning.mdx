...
description: >-
  A failed subtree is a theorem — this partial state can never extend — so
  store it and never re-enter. The community tried both flavours: chess-style
  transposition tables over the search frontier, and mined constraints about
  the puzzle itself. The honest ledger of what memory buys at E2 scale, and
...
sources:
  ...
  - label: "Measured on the 6x6 border ring — 133,326,155 → 30,496,808 iterations (groups.io message 8875)"
  ...
  - label: "The tally — ~68M invalid pairs found, ~8.1×10⁹ undecided, ~32,000 would pin a solution (groups.io message 10973, 2023)"
  ...
  - label: "SAT as an invalidation service — shrinking a dead-end partial to 28 pieces (groups.io message 10292, 2021)"
...
A depth-first search over a scan-line fill order is a function of
...
Peter McGavin caught this happening in the wild in 2016: two top-row
combinations in his 10x10 farming logs used the same ten pieces with the
same permutation of colours along their lower edge, and their search trees
came back with exactly the same node count — 5,602,741,481,523 nodes, twice.
...
is about whether *remembering* is cheaper than *re-deriving* — and at E2
scale that question has a surprisingly consistent answer.
...
supplied the standard engineering answer from computer chess: don't try to
store everything — fix a memory budget, hash the position, overwrite old
entries ([message 6144](https://groups.io/g/eternity2/message/6144)) — and
added a warning that the more the piece sets of two matching frontiers
differ, the shorter the subtree the hit can legally cut.
...
([message 6218](https://groups.io/g/eternity2/message/6218)) — a striking
claim that was never benchmarked by anyone else and never surfaced in any
...
the 52 known solutions. Over thirty-plus messages McGavin built
counterexamples ([message 8866](https://groups.io/g/eternity2/message/8866))
...
[message 8886](https://groups.io/g/eternity2/message/8886)). With the
bookkeeping fixed, the measured saving was real: the 6x6 border ring fell
from 133,326,155 iterations to 30,496,808
...
forget a neighbour edge, a used piece, a restore moment — and you cache a
...
[SAT and CSP encodings page](/research/build/concepts/sat-csp-encodings).
...
possibility matrices with another member promptly caught a rotation bug via
a 618-vs-624 discrepancy
...
still answered UNSAT, and returned a 28-piece core that is already
unextendable ([message 10292](https://groups.io/g/eternity2/message/10292))
— which is conflict-clause minimisation performed by hand, and exactly what
a CDCL solver does internally after every conflict.
...
roughly $8.1 \times 10^9$ pairs still undecided — and by his estimate only
~32,000 *well-chosen* invalidations would pin down a solution
...
the pair space and none of them guaranteed to be among the 32,000 that
matter. Learned constraints about E2 are real assets — they are just being
deposited into an account with astronomical liabilities. Why CDCL's
automated version of this learning also starves on the full board — flat
implication chains, wide late conflicts — is covered on the
[SAT and CSP encodings page](/research/build/concepts/sat-csp-encodings);
...
Verhaard answered plainly: "it does not remember them" — no memory of past
positions at all, and given the node volumes, someone had almost certainly
...
already (see [solver engineering](/research/build/concepts/solver-engineering));
...
through the board — "morality #1: do not care true duplicates". Relaxed
...
- **Small boards and rings.** Lindström's fixed restore-list scheme cut the
...
- **Endgames.** Deep in the board the remaining-piece set is small, the
  ...
  regime where a memo table is small, hot and honest.
- **Inside a propagation engine.** Recording a hard no-good with its full
  ...
  [encodings page](/research/build/concepts/sat-csp-encodings).
