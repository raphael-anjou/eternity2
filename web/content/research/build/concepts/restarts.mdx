...
description: >-
  ...
  just named it. The cure — cut off, reshuffle, restart — is why every record
...
sources:
  ...
  - label: antminder on deadline-aware search — probability in the window beats a better average (groups.io message 3089)
    url: https://groups.io/g/eternity2/message/3089
  - label: Blackwood on the 50-billion cap — "arbitrary" (groups.io message 10066)
    url: https://groups.io/g/eternity2/message/10066
  - label: McGavin's manually restarted 20-PC farm for the 9x9 census (groups.io message 9342)
    url: https://groups.io/g/eternity2/message/9342
  - label: McGavin's 10x10 solve — 92,907 restarted row searches and the law of large numbers (groups.io message 9688)
    url: https://groups.io/g/eternity2/message/9688
  - label: The PRNG hygiene thread — generator period vs restart campaigns (groups.io message 6798)
...
a sceptical Txibilis generated 500 random 7x7 puzzles and raced both policies
...
in satisfiability and CSP search the year before
...
law, $P(T > t) \approx C\,t^{-\alpha}$, not an exponential — so slowly that
for $\alpha \le 1$ the *mean* runtime is infinite. Every Eternity II record
since — Verhaard's 467, McGavin's 10x10, the
[468–470 wave](/research/build/solvers/blackwood) — was found by a solver
built around this fact.

## Why tails happen

A depth-first backtracker with a fixed
[fill order](/research/build/concepts/fill-order) makes its cheapest
...
One honest nuance from the original thread: Txibilis reasoned that restarts
...
is not — the usual case — [Luby, Sinclair and
...
- **Blackwood's engine** caps every attempt at 50 billion nodes and reshuffles
  the opening — first corner, bottom-row pieces — before each new run, so no
  two attempts retrace the same prefix
  ([the solver page](/research/build/solvers/blackwood) covers the
  machinery). Asked how the 50-billion figure was chosen, Blackwood answered:
  "arbitrary. I tried a few big numbers (over 1B) and they didn't make too
...
- **McGavin's farms** are the same policy at the human timescale. His 9x9
  ...
  solve of Brendan Owen's 10x10 — the community's hardest solved benchmark —
...
<Callout kind="note" title="Feed the portfolio honest randomness">
...
calibrated so the shape is honest to the 2007 measurement: a body of lucky
...
rare in a way that does not compound — McGavin's 92,907 10x10 row searches
...
state carries no learning: unlike a clause-learning SAT solver, a plain
backtracker that dies at depth 190 has recorded nothing about *why* — so
persisting preserves no knowledge either.

What is genuinely lost is the certificate. A restart portfolio samples the
tree; it does not sweep it. McGavin says it plainly about his 9x9 census: "I
