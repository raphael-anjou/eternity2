...
Numbers make the trade honest in a way prose cannot. The lab below has two
...
On Eternity II the clean $n$-choices model needs two honest corrections.
First, the interface is not one number but a wide state — a seam vector of
up to 16 edge colours plus the exact piece-pool fingerprint — so the table
