...
sources:
  ...
  - label: sam_maes — the real issue is that good partial solutions are hard to combine (groups.io message 1492)
  ...
  - label: antminder's backtracker+GA hybrid — 462/480 in about a day, Munkres repair operator (groups.io message 5589)
  ...
  - label: jagbrain's 2011 post-mortem — why GA/SA assumptions fail on this search space (groups.io message 8257)
...
reason it fails — a reason worth understanding properly, because it is not
"GAs are weak" but a precise structural clash between crossover and
permutation problems — and then follows the ideas that survived to the pages
where they live now.
...
both. Lyman Hurd stated the premise on the list — and his own doubt —
in one sentence: two partial solutions are likely to need *the same tiles in
different places*
...
poorly aimed mutation — and at that point the population is just a set of
...
The record, in chronological order — it is remarkably consistent.
...
- **2007, the two-week study.** Steve Moyer ran a 1000-individual GA with
  ...
  barely matters — a 100-individual population converged about as fast at a
  tenth of the cost ([message 565](https://groups.io/g/eternity2/message/565)).
...
- **2008, the serious hybrid.** antminder built the strongest evolutionary
  ...
  polishes the remainder — with the disruptive crossover compensated by a
...
- **2008, the shelving.** Three months later antminder reported his program
  ...
  backtracking-based eii "completely blows it away"
...
- **The long tail.** A 2009 blog asked the list what crossover people used;
  ...
  neural-network-with-GA solver was shared with the honest caveat "it wont
...
tabu search, VLNS, [hyper-heuristics](https://link.springer.com/article/10.1007/s10852-012-9178-4)
...
- **The population became restarts.** Once crossover contributes nothing, a
  ...
  restarts — and Moyer's finding that population size barely matters is the
  ...
  the [restarts page](/research/build/concepts/restarts).
- **Mutation + selection + smart repair became ALNS.** antminder's
  ...
  [local search and ALNS](/research/build/concepts/local-search-alns), where
...
- **Per generation: $O(P \cdot (f + g))$** for population size $P$, fitness
  cost $f$ (a linear edge count, cheap) and operator cost $g$ — cheap for
  swap mutations, $O(k^3)$ per Hungarian repair of $k$ cells, and worthless
  in between for repaired crossover. The multiplier $P$ is the painful part:
...
- **No guarantees of any kind.** No completeness, no optimality certificate,
  and — unlike [ALNS](/research/build/concepts/local-search-alns), which at
...
- **The honest verdict.** Pure GAs plateau around 406/480 on the full board
  ([message 2360](https://groups.io/g/eternity2/message/2360)). The best
  hybrid ever reported on the list averaged 462 per day in 2008 by demoting
  the GA to a management layer over a backtracker and an exact repair
  ([message 5589](https://groups.io/g/eternity2/message/5589)) — and its
  author shelved it the week a pure backtracking solver appeared
