...
restore. The removed nodes keep their own pointers while detached — that is
the entire trick — so undoing a cover is as cheap as doing it.
...
2. **Recurse.** Only R1 and R3 survive. Column B now has a single live
   option — choose it, try R3 \{B,C,F\}: cover B, C, F, which detaches R1.
3. **Dead end.** Column E is still uncovered and no live option covers it.
   This branch can never complete — no deeper search is even attempted.
...
6. **No columns remain** — every item covered exactly once. Solution:
   ...
   solution — that certainty of exhaustion is the whole product.

## What it costs

The honest ledger has two very different lines:
...
On Eternity II's scale: the XCC matrix itself is perfectly manageable — 512
