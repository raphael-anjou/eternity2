...
5. **Apply Berge's rule.** An unmatched edge can join *some* maximum matching
   ...
   are deleted — provably, not heuristically.
...
$m\sqrt{n} \approx 1.5$ million edge operations for a full build — small
change on modern hardware, and that is the *worst* case, from scratch, with
maximally loose domains. Inside a search nobody rebuilds: the incremental
versions (Régin 1995; Mehlhorn & Thiel 2000) keep the previous matching,
repair it with a few augmenting paths after each domain change, and re-run
the linear SCC pass — near-linear re-filtering per node in practice. The
...
The honest ledger, though, has a hole in it: Régin's original
