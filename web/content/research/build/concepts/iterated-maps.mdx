...
description: >-
  ...
  fixed points are solutions. Veit Elser's method made the cover of PNAS — and
...
sources:
  ...
  - label: Source code from Veit Elser, with a 5x5 example puzzle, shared in the group files (groups.io message 9362)
  ...
  - label: The SRD edge-swap precedent — solved 8x8, never made 10x10 (groups.io message 9447)
...
This page explains the method properly, because it is genuinely beautiful and
genuinely different from everything else in the catalogue. It then reports,
honestly, what the Eternity II community did with it: a flurry of interest in
...
- $C_2$ — **edges match**: every pair of touching edges agrees in color, and
  the rim is gray. Piece inventory is ignored — cells may hold fractional
...
- $P_1(x)$, the nearest valid piece assignment, is a bipartite matching
  ...
  [local search](/research/build/concepts/local-search-alns).
...
2. **No cost function.** The map does not descend anything. That sounds like
   ...
   [local search](/research/build/concepts/local-search-alns) are places it
...
3. **The iterate lives outside both sets.** $x$ need not satisfy either
   ...
   [LP relaxation](/research/build/concepts/lp-relaxations), it explores a
...
solving and asked whether it had ever been tried
...
3-SAT — scaling comparably to WalkSAT — and used it to improve known sphere
...
a graph that is locally tree-like nowhere — every 2x2 block is a cycle — is
...
The honest record, in full — because it is short.
...
- **2015 — code, and a promise.** Someone obtained source code from Elser
  himself, with a worked 5x5 edge-matching example, and shared it in the
  ...
  standing — it "has solved small model versions of Eternity II," and had it
  ...
  of 2007: capable of 8x8, never 10x10
...
or divide-and-concur on the full 480-edge puzzle, or even on the 10x10
...
3. **Climb the benchmark ladder.** Run against
   [Brendan Owen's puzzle suite](/research/build/benchmarks) — the same 7x7
   where Dima's dual annealer got 48/49, then 8x8, 9x9, 10x10 — with a plain
...
- **Per iteration: two projections.** The edge projection is linear in the
  ...
  second](/research/build/concepts/solver-engineering). Divide-and-concur
...
- **The honest scoreboard.** One community test, lost to a hill-climber on
