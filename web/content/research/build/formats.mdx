...
description: >-
  ...
  exact byte-level rules (how the grey border is encoded in each) and, most of
  all, what each format can and cannot recover.
metaDescription: "The byte-level spec for every Eternity II board and puzzle format: board_edges, board_pieces, e2pieces.txt, Puzzle CSV, site Puzzle JSON, the BoardDoc output and both viewer URLs. How each encodes the grey border, and whether it recovers rotation and piece identity."
kind: reference
order: 8
group: Start here
...
sources:
  - label: "The canonical IO layer that defines these formats (e2-io: the one board format, derived everywhere)"
