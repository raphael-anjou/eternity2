...
<Callout kind="note" title="One canonical output now">
Every algorithm on this site emits **one** artifact per board: a canonical
`.json` document (the `BoardDoc`, below) carrying a ready-to-open
[eternity2.dev](/viewer) URL as a field. Everything else here is either an
*input* the tools accept or a *derived* format the converter can hand you. There
is no longer a `.url`-here, `.json`-there, two-files-sometimes split.
</Callout>
...
## Site JSON (`Puzzle`) — the instance schema

The schema the site's engine and the benchmark's variant files share, so the two
experiments run the *same* instances. It carries the piece set at rotation 0 and
the pinned hints explicitly.
