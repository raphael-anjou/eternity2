...
description: >-
  ...
  ce site et dans la communauté : la chaîne de lettres board_edges et la liste
  d'indices hints, e2pieces.txt, le CSV de puzzle, le JSON Puzzle du site, et
  l'URL de visualiseur, avec les règles exactes au niveau de l'octet (comment la
  bordure grise est encodée dans chacun) et, surtout, ce que chaque format sait
  et ne sait pas restituer.
metaDescription: "La spec au niveau de l'octet de chaque format Eternity II : board_edges, la liste d'indices hints, e2pieces.txt, CSV, JSON Puzzle, BoardDoc et l'URL de visualiseur. Comment chacun encode la bordure grise, et pourquoi les arêtes seules restituent l'identité de pièce et la rotation."
kind: reference
order: 8
group: Start here
...
- **Se marie avec :** `board_edges`. Ensemble ils portent un
  *puzzle-avec-indices* entier dans un seul lien, ce qu'un plateau nu ne
  pourrait jamais.

## board_pieces : le canal d'interopérabilité
...
[boîte à outils](/research/build/tooling) raconte cette histoire). Pleinement
pris en charge en entrée et disponible en sortie dérivée, avec sa paire
`board_w`/`board_h`, un canal `board_pieces` et un fragment `#` :
