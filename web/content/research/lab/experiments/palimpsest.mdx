...
back to [adaptive large-neighbourhood search](/research/build/concepts/local-search-alns).
...
The search that *found* it is a stochastic ALNS run (30 min × 6 seeds); it will
not reproduce the same board, which is why the artifact of record is the board,
not a re-run. The mining scripts and the attack driver live in the vault under
`vol-129`.
