...
metaDescription: "The hint study's findings: on generated Eternity II boards, five clue-shaped hints never help a chronological backtracker — they cost a compact sweep ~30 points and cripple a hint-seeking order by ~330 — because a pinned piece is a hard constraint a fixed fill order must satisfy on arrival."
kind: page
...
board either climbs into the 330s or stalls near 100, with little in between. A
...
the wrong question — "which order best *uses* the hints?" The honest question is
whether the hints help *at all*. The second chart answers it, and the answer is no.
...
order. On the compact sweeps they cost around thirty points. On spiral-out they cost
over two hundred. And on the hint-seeking `connect-hints-first` order they cost
roughly **three hundred and thirty** — the same order scores among the *best* of all
eight with no hints and by far the *worst* with them. Handing the solver five correct
...
collapse to ~100 are precisely the ones where a clue-shape pin lands where the sweep
cannot honour it; the same board with no pins climbs into the high 300s.

This reframes the fill-order result. The order still matters — a compact sweep
survives the hint constraints with a ~30-point scar while `connect-hints-first` is
destroyed by them — but what the order is buying is not "using the hints well." It
