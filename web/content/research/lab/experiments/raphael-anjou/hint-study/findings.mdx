...
and hurts the latter. It is one more instance of the study's throughline, that a
hint's value is set by the search that reads it and the budget it runs under, not
by the hint.
