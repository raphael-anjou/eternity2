...
by the same corner-perm-plus-Hamming test [PRIOR](/research/lab/experiments/raphael-anjou/prior)
uses.

## Reproduce

`just research-record-boards` verifies the committed 458 board's score exactly.
The sweep is stochastic (nine scans × four seeds, then ALNS), so it will not
reproduce the same board; the board is the artifact of record. The scan-sweep
driver lives in the vault under `v175_gauntlet`.
