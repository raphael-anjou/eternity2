...
Five families, laid out so that neighbours differ by a single decision.
...
mechanism the records rely on. The **community** engines (McGavin's C and
Blackwood's C#) were built and run on the same machine, but on their native
instances, because neither can accept our corner-pinned variants. Seeing exactly
how they fail is a result in its own right, so they appear in a reference panel of
their own rather than on the leaderboard.
