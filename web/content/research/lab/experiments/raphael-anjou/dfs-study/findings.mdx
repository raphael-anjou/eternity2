...
metaDescription: "The DFS study's findings: fill order, heuristics and breaks compared. Row-major wins; MRV rescues border-first; breaks smash the depth wall."
kind: page
...
full board, with row-major topping out around depth 208 of 256. The record engines
get past it by *breaking*: allowing a bounded number of interior edge mismatches,
...
## Where the community engines sit, and why they cannot take our grid

Blackwood and McGavin are the high end of this same family, and both build and run
on this machine, so this study ran them; the result is a finding in its own right.
**Neither can take the corner-pinned variants.** McGavin's C reaches depth 205 on
the plain five-clue puzzle at about 110 million tiles per second, roughly three
times our fastest strict engine, yet pinning three corners collapses it to depth
21. Its generated scan path never visits the corners early, so a pinned corner
constrains its neighbourhood at once and dead-ends the fixed path almost
immediately. Blackwood's C# reaches only about 34 of 256 cells on the pinned
five-clue in a minute, because its break heuristic is tuned for the
near-unconstrained one-clue instance where its 470 record was set.
...
does not transfer to another, and a fixed scan path cannot absorb an arbitrary
pin. Our from-scratch engines treat a pin as a pre-placed cell the scan simply
skips, which is why they, rather than the community binaries, are the runnable
stand-ins on this grid. The community engines are reported on their native
instances, in the reference panel above, and not as rows on the leaderboard.
