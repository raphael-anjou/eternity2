...
metaDescription: "The repair study's findings: random destroy beats conflict-driven, construction dominates repair, and every variant stalls early."
kind: page
...
higher still and finishes at a mean of 373, the best of the starting-board family.
The lesson matches the construct-then-refine pipelines behind this project's
record-approaching boards: the board that reaches the top is *built* well and then
repaired, never repaired from nothing.
...
- **A geometry-blind random destroy wins outright, at a mean of 401 (best 409).**
  Lifting twelve *random* cells instead of the broken ones, it keeps sampling
  fresh regions of the board, accepts nearly three quarters of its moves, and its
  best score keeps improving deep into the run: its last improvement lands past
  three million iterations, where every conflict-driven variant has long since
  frozen. It is the highest score in the whole study.
- **Every conflict-driven operator finishes below it, and the more it fixates on
  ...
  still, at 350, and its best last improves at iteration 76: a hole that size hands
...
- **A cooling simulated-annealing rule is the strongest acceptance by a clear
  margin, at 375 (best 388), second only to random destroy in the whole study.**
...
fourteen-point swing on the same loop. That is worth stating against a known
