...
metaDescription: "CLOISTER fixes a perfect Eternity II border, then searches the interior with the border's edges treated as hard constraints from the very first cell. This border-first pipeline reached 453 of 480: a test of how much a locked, matching frame helps or hurts the interior search."
kind: experiment
hardware:
  cores: 8
  cpu: "Apple M1"
...
stages:
  - engine: frame-input
    does: "A pre-solved 60-piece perfect border is fixed as the starting frame"
  - engine: break-dfs
    does: "Break-tolerant DFS fills the 14×14 interior, pruning doomed lines against the frame from cell one"
  - engine: exact-tail
    does: "A 14-cell exact endgame seals the interior against the rim deterministically"
order: 60
updated: 2026-07-21
date: 2026-07
...
frame alongside the engine is planned. For now the run remains exploratory: the
453 board is verifiable in the viewer, but no packaged one-command reproduction
ships yet.
