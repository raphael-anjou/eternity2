...
metaDescription: "GAUNTLET runs one beam search across nine scan orders (row, zigzag, spiral, diagonal…) so it samples different regions instead of converging to one: the zigzag order, lifted with ALNS, reached a new 458 board in an unseen corner family — proof that scan order beats the random seed for diversity."
kind: experiment
hardware:
  cores: 8
  cpu: "Apple M1"
