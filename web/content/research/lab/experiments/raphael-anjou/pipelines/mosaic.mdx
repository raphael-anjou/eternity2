...
metaDescription: "MOSAIC tiles the Eternity II board into sixteen 4×4 blocks and solves each to a proven MaxSAT optimum with core-guided RC2, seams handled as soft clauses so a block never fails. Reserving the ~8% scarcest pieces against piece theft, a 16-level backtracking search reaches 448 of 480 from scratch."
kind: experiment
hardware:
  cores: 8
  cpu: "Apple M1"
...
backing directory for it is planned alongside the other exact experiments. For
now the run remains exploratory: the 448 board is verifiable in the viewer, but
no packaged one-command reproduction ships yet.
